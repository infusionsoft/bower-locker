...
console.log('Start bower-locker ...');
var command = "";

program
    //.arguments('lock | unlock')
    //.option('-v, --verbose', 'The user to authenticate as')
    .command('lock', 'Lock the current bower configuration', {isDefault: true})
    .command('unlock', 'Revert back to standard bower configuration')
    //.action(function(option) {
    //    if (option === "lock" || option === "unlock") {
    //        console.log('option: ', option);
    //        command = option;
    //    } else {
    //        console.error("Invalid command option: ", option);
    //        process.exit(1);
    //    }
    //})
    .parse(process.argv);
